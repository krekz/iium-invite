...
import { Home, Search, MoreHorizontal, Plus, Bookmark, LogOut, User } from "lucide-react";
...
const MobileNavbar = ({ session }: { session: { user: string | undefined | null } }) => {
    ...
    const MobileNavLinks = ({ isActive, href, icon: Icon, label }: { isActive: boolean, href: string, icon: React.ElementType, label: string }) => (
        <Link href={href} className={cn("flex flex-col items-center gap-0.5", {
    ...
    if (!session.user) {
        return (
            <div className="fixed bottom-0 z-50 left-0 right-0 bg-background border-t sm:hidden">
                            ...
                            {[
                                {
                                    ...
                                    isActive: pathname === "/account" && !searchParams.get("option")
                                },
                                {
                                    href: "/account?option=posts",
                                    icon: Plus,
                                    label: "My Posts",
                                    isActive: pathname === "/account" && searchParams.get("option") === "posts"
                                },
                                {
                                    href: "/account?option=bookmarks",
                                    icon: Bookmark,
                                    label: "Bookmarks",
                                    isActive: pathname === "/account" && searchParams.get("option") === "bookmarks"
                                }
                            ].map(({ href, icon: Icon, label, isActive }) => (
                                <SheetClose key={href} asChild>
                                    <Link href={href} className={cn("flex items-center gap-2 px-4 py-2 hover:bg-accent rounded-md", {
                                        "text-amber-600": isActive
                                    })}>
                                        <Icon size={16} />
                                        <span>{label}</span>
